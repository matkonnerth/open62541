...
def generateVariableTypeNodeCode(node, nodeset, encode_binary_size):
    ...
    if node.dataType is not None:
        ...
        if dataTypeNode is not None:
            ...
            if dataTypeNode.isEncodable():
                if node.value is not None:
                    [code1, codeCleanup1, codeGlobal1] = generateValueCode(node.value, nodeset.nodes[node.id])
                    code += code1
                    codeCleanup += codeCleanup1
                    codeGlobal += codeGlobal1
                else:
            ...
            if isinstance(subv, list):
                ...
                code.append(
                     "{0}->{1} = (UA_{2}*) UA_malloc(sizeof(UA_{2})*{3});".format(
                         instanceName, subv.alias[0].lower()+ subv.alias[1:], subv.__class__.__name__, str(len(subv))))
                ...
                for subvidx in range(0, len(subv)):
                    ...
                    valueName = instanceName + "->" + subv.alias[0].lower()+ subv.alias[1:] + "[" + str(
                        subvidx) + "]"
